...
@export var sm_atk1dmg = Global.dmg + 15
@export var sm_atk2dmg = Global.dmg + 20
@export var sm_atk3dmg = Global.dmg + 25
...
func attack():
	...
	if Input.is_action_just_pressed("atk1"):
		...
		if dir == "right":
			...
			$AnimatedSprite2D.play("swordsman_atk1")
	...
	if Input.is_action_just_pressed("atk2") and not atk2_cooldown:
		...
		if dir == "right":
			$AnimatedSprite2D.flip_h = false
			$AnimatedSprite2D.play("swordsman_atk2")
			$atk_hitbox.scale.x = 1
			$deal_attack_timer.start()
		elif dir == "left":
			$AnimatedSprite2D.flip_h = true
	...
	if Input.is_action_just_pressed("atk3") and not atk3_cooldown:
		...
		if dir == "right":
			$AnimatedSprite2D.flip_h = false
			$AnimatedSprite2D.play("swordsman_atk3")
			$deal_attack_timer.start()
		elif dir == "left":
			$AnimatedSprite2D.flip_h = true
			$AnimatedSprite2D.play("swordsman_atk3")
...
func _on_atk_hitbox_area_entered(area):
	if area.name == "hitbox_area":
		...
		print("THE PARENT NODE IS: ", parent_node)
		parent_node.deduct_hp(damage)
		print("THE PARENT NODE HEALTH IS: ", parent_node.health)
